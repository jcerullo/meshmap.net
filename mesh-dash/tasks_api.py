...
DATABASE_FILE = "tasks.db" # Consider making this configurable via environment or main app config
logger = logging.getLogger("meshtastic_dashboard.tasks") # Matches the logger name in the scheduler
    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS tasks (
                ...
                enabled BOOLEAN DEFAULT TRUE,  -- Added 'enabled' column
        ...
        # Check if 'enabled' column exists and add it if not (for existing databases)
...
class TaskBase(BaseModel):
    ...
    cronString: str
    enabled: Optional[bool] = True # Default to True for new tasks if not specified
...
class TaskUpdate(BaseModel):
    ...
    enabled: Optional[bool] = None # Allow updating the enabled status

class TaskInDB(TaskBase):
    """Model representing a task as stored in and retrieved from the database."""
    id: int
    createdAt: str # SQLite TIMESTAMP is often returned as string
    updatedAt: str # SQLite TIMESTAMP is often returned as string
    enabled: bool # Ensure enabled is always present and boolean in response
...
async def create_task(task: TaskCreate, conn: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        cursor = conn.cursor()
        # Ensure 'enabled' is explicitly handled, defaulting to True if not in task payload
        # Pydantic model default handles this if task.enabled is None
        enabled_value = task.enabled if task.enabled is not None else True

        cursor.execute(
            """
            INSERT INTO tasks (nodeId, taskType, actionPayload, cronString, enabled)
            VALUES (?, ?, ?, ?, ?)
            """,
            (task.nodeId, task.taskType, task.actionPayload, task.cronString, enabled_value)
        )
        conn.commit()
        new_task_id = cursor.lastrowid
        if new_task_id is None: # Should not happen with AUTOINCREMENT if insert was successful
        ...
        # Fetch the newly created task to return it, including all fields like createdAt, updatedAt, and enabled
        cursor.execute("SELECT id, nodeId, taskType, actionPayload, cronString, enabled, createdAt, updatedAt FROM tasks WHERE id = ?", (new_task_id,))
        created_task_row = cursor.fetchone()

        if created_task_row:
            created_task_dict = dict(created_task_row)
            logger.info(f"Created task with ID: {new_task_id}, Enabled: {created_task_dict.get('enabled')}")
            return TaskInDB(**created_task_dict)
        else:
            # This case should be rare if the insert succeeded and ID was obtained
    ...
    except sqlite3.IntegrityError as e: # E.g., if there was a UNIQUE constraint violated (not in current schema)
...
async def get_all_tasks(conn: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        ...
        cursor.execute("SELECT id, nodeId, taskType, actionPayload, cronString, enabled, createdAt, updatedAt FROM tasks ORDER BY createdAt DESC")
...
async def get_task(task_id: int = Path(..., description="The ID of the task to retrieve"), conn: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        cursor = conn.cursor()
        # Ensure 'enabled' column is selected
        cursor.execute("SELECT id, nodeId, taskType, actionPayload, cronString, enabled, createdAt, updatedAt FROM tasks WHERE id = ?", (task_id,))
